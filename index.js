...
const cake = createMenuItem("cake", 5, "Dessert");
const french_toast = createMenuItem("french toast", 7, "Breakfast");
console.log(pizza, cake, french_toast);
...
reviews.push({name: "Lisa", rating: 1, feedback: "Couldn't eat a thing."});
console.log(reviews);
...
function getReviewByIndex(review_array, index) {
  return `${review_array[index].name} gave the restaurant a ${review_array[index].rating} star review, and their feedback was: ${review_array[index].feedback}`;
}
...
function getLastReview(review_array) {
  return getReviewByIndex(review_array, review_array.length-1); 
} 
...
function carMaker(miles_driven) {
  return {
    odometer: miles_driven,
...
let car = carMaker(10); console.log(car);
console.log(car.drive(100)); console.log(car);
